...
RSpec.describe User, type: :model do
  describe 'Validations' do
    ...
    it 'should validate password' do
      @user = User.create(name: "Test", email: "test@test.com", password: nil, password_confirmation: "password")
    ...
    it 'should validate password confirmation' do
      @user = User.create(name: "Test", email: "test@test.com", password: "password", password_confirmation: "")
      expect(@user.errors.full_messages).to eq(["Password confirmation doesn't match Password"])
    end

    it 'should make sure that password and password_confirmation are the same' do
      @user = User.create(name: "Test", email: "test@test.com", password: "password", password_confirmation: "1234")
      expect(@user.errors.full_messages).to eq(["Password confirmation doesn't match Password"])
    end
